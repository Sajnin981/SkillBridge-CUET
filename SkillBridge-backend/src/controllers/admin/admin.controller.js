 ...
 * Removes an account and records that cannot remain valid without it.
 */
exports.deleteUser = async (req, res, next) => {
  try {
    ...
    const accountModel = role === "student" ? "Student" : "Company";
    const accountFilter = { [role]: item._id };
    const conversations = await Conversation.find(accountFilter).select("_id");
    const conversationIds = conversations.map((conversation) => conversation._id);
    const applicationFilter = role === "student" ? { student: item._id } : { company: item._id };
    const applications = await Application.find(applicationFilter).select("_id");
    const applicationIds = applications.map((application) => application._id);

    if (role === "company") {
      const opportunities = await Opportunity.find({ company: item._id });
      await Promise.all(opportunities.map((opportunity) => opportunity.deleteOne()));
    }

    await Promise.all([
      Application.deleteMany(applicationFilter),
      Message.deleteMany({ conversation: { $in: conversationIds } }),
      Conversation.deleteMany(accountFilter),
      Notification.deleteMany({ $or: [{ recipient: item._id, recipientModel: accountModel }, { applicationId: { $in: applicationIds } }] }),
      AILog.deleteMany({ requester: item._id, requesterModel: accountModel }),
      item.deleteOne(),
    ]);

    return success(res, { message: `${role === "student" ? "Student" : "Company"} deleted`, data: {} });
  } catch (err) {
    next(err);
  }
};
