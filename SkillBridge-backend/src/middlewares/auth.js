...
/**
 * Convenience: require an approved company before it can access protected
 * resources. Students do not require admin verification.
 */
const requireApproved = (req, _res, next) => {
  const user = req.user;
  if (req.userRole === "admin") return next();

  if (req.userRole === "company" && user.status && user.status !== "approved") {
    return next(
      new AppError(
